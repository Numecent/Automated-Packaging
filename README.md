...
Numecent does not offer professional support services for automated packaging at this time. All support efforts are community driven. Please visit our [community discussion forum here](https://github.com/Numecent/Automated-Packaging/discussions/).
...
The Cloudpaging Studio is where the science begins in the form of application packaging. The Studio prepares the application for automated deployment, updates, and access settings based upon the predetermined permission levels within your organization. 
Supported operating systems: Windows 10 & 11 and PowerShell version 5.1.
...
1. Open PowerShell as an administrator in your virtual environment and run the following command:
```
...
4. Now the PowerShell script can be executed using the following command:
```
...
The file `Example_JSON_1.3.json` details the basic format of the JSON config instruction file.
...
`ProjectName` is the name of the project. If a project file with the same name does not exist, it will be created, otherwise the existing project file will be loaded and saved with the new settings.
...
```
    "PostCaptureCommands": {
        "AddFolders": {
            "Folder1": {
                "SourcePath": "C:\\Program Files\\WinRAR",
                "DestinationPath": "",
                "IncludeSubfolders": true,
                "IncludeFilesInFolders": false
            },
            "Folder2": {
                "SourcePath": "C:\\Program Files\\VLC",
                "DestinationPath": C:\\VLC"",
                "IncludeSubfolders": true,
                "IncludeFilesInFolders": true
            }
        },
        "AddKeys": {
            "Key1": {
                "SourceKey": "HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Silverlight",
                "DestinationKey": "",
                "IncludeSubkeys": true,
                "IncludeValuesInKeys": true
            },
            "Key2": {
                "SourceKey": "HKEY_LOCAL_MACHINE\\Software\\Microsoft\\Silverlight",
                "DestinationKey": "HKEY_LOCAL_MACHINE\\SOFTWARE\\WOW6432Node\\Microsoft\\Silverlight",
                "IncludeSubkeys": false,
                "IncludeValuesInKeys": false
            }
        },
        "MergedFolders": {
            "MergedFolder1": {
...
```
PostCaptureCommands section contains the folders, merged folders properties and keys that will be added to the package after the capture process is complete.

**AddFolders:** `SourcePath` and `DestinationPath` are the path to the source and destination folders respectively. When `DestinationPath` is empty, it is assumed to be the same as `SourcePath`.
`IncludeSubfolders` and `IncludeFilesInFolders` allow to recursively include subfolders and files to the package.
...
**MergedFolders:** `Path` is the path to the root folder in the project. When `Merged` is empty, it is assumed it's **true**. Merged Folder feature required Studio 9.4.1 or greater.
...
The possible values for `CompressionMethod` are: “None”, “LZMA”, “LZ4”.

The possible values for `EncryptionMethod` are: “None”, ”AES-256-Enhanced”.
