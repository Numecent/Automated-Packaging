...
```
PostCaptureCommands section contains the folders and keys that will be added to the package after the capture process is complete.

`SourcePath` and `DestinationPath` are the path to the source and destination folders respectively. When `DestinationPath` is empty, it is assumed to be the same as `SourcePath`.
`IncludeSubfolders` and `IncludeFilesInFolders` allow to recursively include subfolders and files to the package.

`SourceKey` and `DestinationKey` are the path to source and destination registry location respectively. When `DestinationKey` is empty, it is assumed to be the same as `SourceKey`.
