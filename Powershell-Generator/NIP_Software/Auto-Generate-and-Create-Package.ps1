...
#  Updated: 2023-09-05
#    	Added sandbox exclusion parameters
#
########################################################################
...
if ($PSBoundParameters.Count -ge 1 -or $Args.Count -ge 1) {
    if (!$filepath) {
        Write-Error 'Please set a filepath to the msi/exe/cmd/bat/ps1 you wish to package' -ErrorAction stop
    }
